...
BEGIN
    ...
    SELECT art.idArt, art.title, art.description, art.content, art.image, art.slug AS article_slug, art.category_id, DATE_FORMAT(art.created_at, 'Le %d/%m/%Y à %H:%i') AS date_fr, cat.category_name, cat.slug AS category_slug, u.pseudo, u.email
    FROM article AS art
